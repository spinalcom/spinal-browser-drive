...
angular.module('app.directives').directive('navbar',[function(){return{restrict:'E',templateUrl:'app/templates/navbar.html',controller:'navbarCtrl'}}]).directive('menuGlayout',['goldenLayoutService','$timeout',function(a){return{restrict:'E',scope:{layoutInfo:'=info'},replace:!0,template:'<li ng-repeat="layout in layoutInfo"  id="{{layout.id}}"><a >{{layout.name}}</a></li>',link:(b)=>{a.wait_ready().then(()=>{let c=(a,b)=>()=>{a.createChild(b.cfg)};a.watch_pannel((d)=>{for(var e=0;e<b.layoutInfo.length;e++){let d=b.layoutInfo[e];d.shown||(d.shown=!0,a.createDragSource($('#'+d.id)[0],d.cfg),$('#'+d.id).click(c(a,d)))}b.layoutInfo=d,b.$apply()})})}}}]).directive('ngRightClick',['$parse',function(a){return function(b,c,d){var e=a(d.ngRightClick);c.bind('contextmenu',function(a){b.$apply(function(){a.preventDefault(),e(b,{$event:a})})})}}]);
...
angular.module('app.controllers').controller('navbarCtrl',['$scope','authService','$location',function(a,b,c){a.username='',a.connected=!1,b.wait_connect().then(()=>{a.username=b.get_user().username,a.connected=!0}),a.logout=()=>{c.path('/login')},a.layouts=[{id:'drag-folder-explorer',name:'Folder Explorer',cfg:{isClosable:!0,title:'Folder Explorer',type:'component',width:20,componentName:'SpinalHome',componentState:{template:'sideBar.html',module:'app.sidebar',controller:'sideBarCtrl'}}},{id:'drag-file-explorer',name:'File Explorer',cfg:{isClosable:!0,title:'File Explorer',type:'component',componentName:'SpinalHome',componentState:{template:'FileExplorer.html',module:'app.FileExplorer',controller:'FileExplorerCtrl'}}},{id:'drag-inspector',name:'Inspector',cfg:{isClosable:!0,title:'Inspector',type:'component',componentName:'SpinalHome',componentState:{template:'inspector.html',controller:'InspectorCtrl'}}}];for(var d=0;d<a.layouts.length;d++)goldenLayoutService.registerPannel(a.layouts[d])}]);
